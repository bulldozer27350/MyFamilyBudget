...
- **Limites connues** : taux fixe uniquement (pas de taux variable, différé, remboursement anticipé) ; l'analyse des prêts côté serveur (`LoanAdviceCalculationService`) ne connaît pas encore le palier de mensualité lissée. Les projections du CRD (`projectLoanCrdToDate` et la boucle de trajectoire de `calculateDetailedFinancialTimeline`, dans `calculations.js`) le connaissent : après la fin du palier, la mensualité est recalculée pour solder le prêt à sa date de fin (`paymentAfterStep`).
